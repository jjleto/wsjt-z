      ...
      if (target_rx_bands.size () + target_tx_bands.size ())
        {
          ...
          if (!(m_->rx_permutation_.size () + m_->tx_permutation_.size ()) // all used up
              ...
              || !target_tx_bands.contains (tx_permutations))
            {
              ...
              std::shuffle (std::begin (m_->rx_permutation_), std::end (m_->rx_permutation_), std::mt19937 {std::random_device {} ()});
              m_->tx_permutation_ = target_tx_bands.values ();
              std::shuffle (std::begin (m_->tx_permutation_), std::end (m_->tx_permutation_), std::mt19937 {std::random_device {} ()});
